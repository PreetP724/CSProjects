...
public class Tournament {
	...
	public int numLess = 0;
	public int numExch = 0;
	...
	static int model_S(int N) {
		return (int) (2 * N * ((Math.log(N)/Math.log(2)) - 2) + 2 * (Math.log(N)/Math.log(2)) + 5);
	}
	...
	static void trial(int type) {
		...
		for (int size = 64; size <= 65536; size *= 2) {
			...
			int numLess = heap.numLess;     // DO SOMETHING ELSE!!!
			int numExch = heap.numExch;
			...
			System.out.println(String.format("%10d %10d %10d %10d %10d %10d", size, max, second, numLess, numExch, (numLess+numExch)));
		}
	}
