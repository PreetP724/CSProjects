...
Stack * create (int max_cells) {
  ...
  stack->elements = (void**) malloc(max_cells * (sizeof(void*)));
  stack->max_cells = max_cells; //set max_cells field
  // Valid stack was allocated, initialize attributes
  // Note: because we called calloc, fields initialized to 0
  stack->top = stack->base = stack->elements;
...
void * push (Stack *stack, void *element) {
  ...
  size_t maxelements = stack->max_cells;
  if (maxelements == stack->num_elements) {
    return (void *) NULL;
  }
